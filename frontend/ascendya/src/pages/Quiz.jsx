...
const stepSchemas = {
  ...
  5: yup.object({
    studyExpectation: yup.string().required('Selecciona tu expectativa de estudio'),
    timeCommitment: yup.string().required('Selecciona tu disponibilidad de tiempo')
  })
};
...
function Quiz() {
  ...
  const resourceTypes = [
    { id: 'videos', name: 'Videos educativos', icon: '🎥', description: 'Contenido visual y dinámico' },
    { id: 'articulos', name: 'Artículos y blogs', icon: '📄', description: 'Lectura profunda' },
    { id: 'cursos', name: 'Cursos interactivos', icon: '🎓', description: 'Aprendizaje guiado' },
    { id: 'libros', name: 'Libros digitales', icon: '📚', description: 'Teoría fundamentada' },
    { id: 'podcasts', name: 'Podcasts', icon: '🎧', description: 'Aprendizaje auditivo' },
    { id: 'ejercicios', name: 'Ejercicios prácticos', icon: '💪', description: 'Práctica dirigida' },
    { id: 'simuladores', name: 'Simuladores', icon: '🕹️', description: 'Experiencia inmersiva' },
    { id: 'documentales', name: 'Documentales', icon: '🎬', description: 'Casos reales' }
  ];

  const vocationalAreas = [
    { id: 'ciencia_tecnologia', name: 'Ciencia y Tecnología', icon: '🔬', description: 'Investigación, innovación, desarrollo' },
    { id: 'artes_humanidades', name: 'Artes y Humanidades', icon: '🎨', description: 'Creatividad, cultura, expresión' },
    { id: 'ciencias_sociales', name: 'Ciencias Sociales', icon: '👥', description: 'Sociedad, comportamiento, política' },
    { id: 'salud_medicina', name: 'Salud y Medicina', icon: '⚕️', description: 'Bienestar, tratamiento, prevención' },
    { id: 'negocios_economia', name: 'Negocios y Economía', icon: '💼', description: 'Empresas, finanzas, mercados' },
    { id: 'ingenieria', name: 'Ingeniería', icon: '⚙️', description: 'Construcción, sistemas, soluciones' },
    { id: 'informatica', name: 'Informática y Software', icon: '💻', description: 'Programación, sistemas, apps' },
    { id: 'educacion', name: 'Educación', icon: '👨‍🏫', description: 'Enseñanza, formación, desarrollo' }
  ];
  ...
  const generateRouteDescription = (data) => {
    return `Ruta personalizada basada en tus intereses en ${data.academicInterests.join(', ')} y tu preferencia por el aprendizaje ${data.learningStyle.toLowerCase()}.`;
  };
  ...
  const generateCareerPaths = (data) => {
    const careerMap = {
      'informatica': ['Desarrollador de Software', 'Ingeniero de Datos', 'Ciberseguridad'],
      'ciencia_tecnologia': ['Investigador', 'Científico de Datos', 'Ingeniero de I+D'],
      'salud_medicina': ['Médico', 'Enfermero', 'Investigador Biomédico'],
      'negocios_economia': ['Analista Financiero', 'Emprendedor', 'Consultor de Negocios']
    };
  ...
  const generateResources = (data) => {
    return data.resourcePreferences?.map(type => ({
      type,
  ...
  const calculateDifficulty = (data) => {
    const practicalWeight = data.learningStyle === 'muy_practico' ? 1 : 
                           data.learningStyle === 'practico' ? 2 :
                           data.learningStyle === 'equilibrado' ? 3 :
                           data.learningStyle === 'teorico' ? 4 : 5;
    
    return practicalWeight <= 2 ? 'Principiante' : 
           practicalWeight <= 4 ? 'Intermedio' : 'Avanzado';
  };
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>{quizResults.courses.length} cursos incluidos</span>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {quizResults.resources.map((resource, index) => {
                const resourceData = resourceTypes.find(r => r.id === resource.type);
                return (
                  <div key={index} className="text-center p-4 bg-slate-800 rounded-lg">
                    <div className="text-2xl mb-2">{resourceData?.icon}</div>
                    <div className="font-medium text-sm mb-1">{resourceData?.name}</div>
                    <div className="text-xs text-slate-400">{resource.count} disponibles</div>
                  </div>
                );
              })}
            </div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold mb-4">Generando tu ruta personalizada...</h2>
          <p className="text-slate-400">Analizando tus respuestas y creando el mejor camino para ti</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-4">Test Vocacional Personalizado</h1>
        ...
        {/* Formulario */}
        <form onSubmit={handleSubmit(onSubmit)} className="bg-slate-900 border border-slate-700 rounded-xl p-8">
          
          {/* Paso 1: Intereses Académicos */}
          {currentStep === 1 && (
            <div>
              <h2 className="text-2xl font-bold mb-2">¿Qué materias disfrutas más?</h2>
              <p className="text-slate-400 mb-6">Selecciona todas las que te interesen</p>
              
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {academicSubjects.map((subject) => {
                  const isSelected = watchedValues.academicInterests?.includes(subject.id);
                  return (
                    <button
                      key={subject.id}
                      type="button"
                      onClick={() => handleArrayToggle('academicInterests', subject.id)}
              ...
              {errors.academicInterests && (
                <p className="mt-4 text-sm text-red-400">{errors.academicInterests.message}</p>
              )}
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl font-bold mb-2">¿Cómo prefieres aprender?</h2>
                <p className="text-slate-400 mb-6">Cuéntanos sobre tu estilo de aprendizaje</p>
                
                {/* Estilo de aprendizaje */}
                <div className="mb-8">
                  <label className="block font-medium mb-4">¿Prefieres aprender de forma práctica o teórica?</label>
                  ...
                  {errors.learningStyle && (
                    <p className="mt-2 text-sm text-red-400">{errors.learningStyle.message}</p>
                  )}
                </div>

                {/* Trabajo en equipo */}
                <div className="mb-8">
                  <label className="block font-medium mb-4">¿Te gusta trabajar en equipo o individualmente?</label>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      { value: 'equipo', label: 'Prefiero trabajo en equipo' },
                      { value: 'individual', label: 'Prefiero trabajo individual' },
                      { value: 'ambos', label: 'Me adapto a ambos' }
                    ].map((option) => (
                      <label key={option.value} className="flex items-center gap-2 cursor-pointer p-3 border border-slate-600 rounded-lg hover:border-slate-500">
                        <input
                          type="radio"
                          {...register('teamWork')}
                          value={option.value}
                  ...
                  {errors.teamWork && (
                    <p className="mt-2 text-sm text-red-400">{errors.teamWork.message}</p>
                  )}
                </div>

                {/* Actividades preferidas */}
                <div>
                  <label className="block font-medium mb-4">¿Qué tipo de actividades disfrutas más?</label>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {activities.map((activity) => {
                      const isSelected = watchedValues.preferredActivities?.includes(activity.id);
                      return (
                        <button
                          key={activity.id}
                          type="button"
                          onClick={() => handleArrayToggle('preferredActivities', activity.id)}
                  ...
                  {errors.preferredActivities && (
                    <p className="mt-4 text-sm text-red-400">{errors.preferredActivities.message}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {currentStep === 3 && (
            <div>
              <h2 className="text-2xl font-bold mb-2">¿Qué tipo de recursos prefieres para aprender?</h2>
              <p className="text-slate-400 mb-6">Selecciona los formatos que más te gusten</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {resourceTypes.map((resource) => {
                  const isSelected = watchedValues.resourcePreferences?.includes(resource.id);
                  return (
                    <button
                      key={resource.id}
                      type="button"
                      onClick={() => handleArrayToggle('resourcePreferences', resource.id)}
              ...
              {errors.resourcePreferences && (
                <p className="mt-4 text-sm text-red-400">{errors.resourcePreferences.message}</p>
              )}
            </div>
          )}
          ...
          {currentStep === 4 && (
            <div>
              <h2 className="text-2xl font-bold mb-2">¿Qué áreas te interesan más?</h2>
              <p className="text-slate-400 mb-6">Selecciona hasta 3 áreas de tu interés</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {vocationalAreas.map((area) => {
                  const isSelected = watchedValues.vocationalAreas?.includes(area.id);
                  const isDisabled = !isSelected && (watchedValues.vocationalAreas?.length >= 3);
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={isDisabled}
                      onClick={() => handleArrayToggle('vocationalAreas', area.id)}
                      className={`p-4 rounded-lg border-2 transition-all text-left ${
                        isSelected
                          ? 'border-green-500 bg-green-500/20'
                          : isDisabled
              ...
              <div className="mt-4 text-sm text-slate-400">
                {watchedValues.vocationalAreas?.length || 0} de 3 áreas seleccionadas
              </div>
              
              {errors.vocationalAreas && (
                <p className="mt-2 text-sm text-red-400">{errors.vocationalAreas.message}</p>
              )}
            </div>
          )}
          ...
          {currentStep === 5 && (
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl font-bold mb-2">¿Cuáles son tus expectativas?</h2>
                <p className="text-slate-400 mb-6">Cuéntanos sobre tus planes y disponibilidad</p>
                
                {/* Expectativas de estudio */}
                <div className="mb-8">
                  <label className="block font-medium mb-4">¿Te interesa estudiar en universidad o prefieres rutas autodidactas?</label>
                  <div className="space-y-3">
                    {[
                      { value: 'universidad', label: 'Quiero prepararme para la universidad', desc: 'Recursos enfocados en exámenes de admisión' },
                      { value: 'autodidacta', label: 'Prefiero aprendizaje autodidacta', desc: 'Rutas completamente gratuitas y flexibles' },
                      { value: 'ambos', label: 'Me interesan ambas opciones', desc: 'Combinación de preparación universitaria y habilidades prácticas' }
                    ].map((option) => (
                      <label key={option.value} className="flex items-start gap-3 cursor-pointer p-4 border border-slate-600 rounded-lg hover:border-slate-500">
                        <input
                          type="radio"
                  ...
                  {errors.studyExpectation && (
                    <p className="mt-2 text-sm text-red-400">{errors.studyExpectation.message}</p>
                  )}
                </div>

                {/* Tiempo disponible */}
                <div>
                  <label className="block font-medium mb-4">¿Cuánto tiempo puedes dedicar al estudio por semana?</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      { value: '1-2_horas', label: '1-2 horas por semana', desc: 'Ritmo relajado' },
                      { value: '3-5_horas', label: '3-5 horas por semana', desc: 'Ritmo moderado' },
                      { value: '6-10_horas', label: '6-10 horas por semana', desc: 'Ritmo activo' },
                      { value: 'mas_10_horas', label: 'Más de 10 horas por semana', desc: 'Ritmo intensivo' }
                    ].map((option) => (
                      <label key={option.value} className="flex items-start gap-3 cursor-pointer p-4 border border-slate-600 rounded-lg hover:border-slate-500">
                        <input
                          type="radio"
                  ...
                  {errors.timeCommitment && (
                    <p className="mt-2 text-sm text-red-400">{errors.timeCommitment.message}</p>
                  )}
                </div>
              </div>
            </div>
